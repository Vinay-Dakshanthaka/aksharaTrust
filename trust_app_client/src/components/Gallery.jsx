    ...
    useEffect(() => {
        const fetchMedia = async () => {
            try {
                ...
                const galleryOnly = mediaList.filter(item => {
                    return item.category?.toLowerCase() === 'gallery' && item.visible === true;
                });                
                setGalleryItems(galleryOnly);
            } catch (error) {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                {paginatedItems.map((item, index) => (
                    ...
                    >
                        ...
                        {/* Hover Overlay */}
                        {(item.event_name || item.description) && (
                            <div className="absolute inset-0 bg-black bg-opacity-60 rounded-lg flex flex-col justify-center items-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 backdrop-blur-sm text-white text-center p-4">
                                {item.event_name && (
                                    <h3 className="text-lg font-bold">{item.event_name}</h3>
                                )}
                                {item.description && (
            ...
            {lightboxItem && (
                ...
                >
                    <div className="max-w-3xl w-full p-4">
                        ...
                        {(lightboxItem.event_name || lightboxItem.description) && (
                            <p className="mt-4 text-white text-center">
                                <strong>{lightboxItem.event_name}</strong> – {lightboxItem.description}
                            </p>
                        )}
                    </div>
                </div>
